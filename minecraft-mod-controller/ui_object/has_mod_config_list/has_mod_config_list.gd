...
func generate_modcfg_object(data: Array[ModConfigData]) -> void:
	for i in data:
		var obj: ModConfigObject = modConfigObjStyle.instantiate()
		obj.set_data(i.name, i.description, i.mc_version, i.name)
...
func reload_modcfg_object(data: Array[ModConfigData]):
	clear_modcfg_object()
